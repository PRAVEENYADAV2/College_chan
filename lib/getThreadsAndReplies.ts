...
export default async function getThreadsAndReplies(thread_id: number) {
    try {
        ...
        const [rows] : any[]= await pool.query('SELECT * FROM threads WHERE id = ?', [thread_id]);
        const [replies] : any[] = await pool.query('SELECT * FROM replies WHERE thread_id = ?', [thread_id]);
